...
const BACKEND_URL = 'https://loomdev-production.up.railway.app';
...
async function handleFolderSelection(eventDetail) {
  ...
  try {
    const response = await fetch(`https://loomdev-production.up.railway.app/doc/${doc_id}/ontology`);
    if (response.ok) {
...
async function renderDocumentMode(docId) {
  ...
  try {
    ...
    const [textResponse, ontologyResponse] = await Promise.all([
      fetch(`https://loomdev-production.up.railway.app/doc/${docId}/text`),
      fetch(`https://loomdev-production.up.railway.app/doc/${docId}/ontology`)
    ]);
